...
label0:
	...
	jalr	$t1 
	lw	$a0 16($fp) 		# load [x], class cool.structure.Param
	sw	$a0 0($sp) 		# call: Push parameter
	addiu	$sp $sp -4 
	move	$a0 $s0 
...
label2:
	...
	lw	$t1 12($t1) 		# call: method out_string is offset 3
	jalr	$t1 
	lw	$a0 12($fp) 		# load [y], class cool.structure.Param
	sw	$a0 0($sp) 		# call: Push parameter
	addiu	$sp $sp -4 
	move	$a0 $s0 
...
label4:
	...
	addiu	$sp $sp 20 		# m: restore sp, 8 from formals, 12 from local frame
	jr	$ra 
Main.main:
	...
	move	$s0 $a0 		# m: self to $s0
	la	$a0 int_const0 		# 2
	sw	$a0 0($fp) 		# letd: Store initial value of x
	la	$a0 int_const1 		# 3
...
label5:
	lw	$t1 8($a0) 		# call: ptr to dispatch table
	lw	$t1 28($t1) 		# call: method f is offset 7
	jalr	$t1 
	lw	$a0 0($fp) 		# load [x], class cool.structure.Local
	sw	$a0 0($sp) 		# call: Push parameter
	addiu	$sp $sp -4 
	move	$a0 $s0 
