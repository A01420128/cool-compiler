...
Main.recite:
    ...
    lw      $a0     12($fp)            #obj: load [value], Int
...
label2:
    ...
    lw      $t1    8($a0)                 #call: ptr to dispatch table
    lw      $t1    28($t1)              #call: method recite is at offset 28
    jalr    $t1
...
label11:
    ...
    blt     $t1    $t2    label12       #<: exit if less
    la      $a0    bool_const0             #<: load false
label12:


    la      $a0     int_const12           #literal, 11
    ...
    bne     $a0    $zero      label13      #call: protect from dispatch to void
    la      $a0    str_const1               #call: constant object with name of the file
    li      $t1    23                   #call: line number
    jal    _dispatch_abort                  #call: message and die
label13:
    ...
    bne     $a0    $zero      label14      #call: protect from dispatch to void
    la      $a0    str_const1               #call: constant object with name of the file
    li      $t1    23                   #call: line number
    jal    _dispatch_abort                  #call: message and die
label14:
    ...
    la      $a0    bool_const1             #=: load true
    beq     $t1    $t2    label15          #=: if identical (same address)
...
label15:
    ...
    bne     $a0    $zero      label16      #call: protect from dispatch to void
    la      $a0    str_const1               #call: constant object with name of the file
    li      $t1    24                   #call: line number
    jal    _dispatch_abort                  #call: message and die
label16:
    ...
    bne     $a0    $zero      label17      #call: protect from dispatch to void
    la      $a0    str_const1               #call: constant object with name of the file
    li      $t1    24                   #call: line number
    jal    _dispatch_abort                  #call: message and die
label17:
    ...
    lw      $t2    12($a0)                #<:
    la      $a0    bool_const1             #<: load true
    ble     $t1    $t2    label18       #<: exit if less
    la      $a0    bool_const0             #<: load false
label18:
